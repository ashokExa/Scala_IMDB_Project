...
class SuperMoviesTest extends AnyFunSuite{
  ...
  test("verify that file read success and  got the result properly") {
    ...
    assert(result(0).equals("Kantara,2022,Rishab Shetty,\"Rishab Shetty, Sapthami Gowda, Kishore Kumar G., Achyuth Kumar\",9.3,148,UA,Gross Unkown,Action,\" Adventure,  Drama\""))
  }
  test("verify that output movies data written in the output path properly") {
    var writtenStatus = superMovies.writeMovieDataBasedOnLogic(s"/Users/ramachandiran.devan/Documents/DMD/scala_file/Movie_Data_By_Rating.csv",result ,HandleMoviesData.outputByHighestRating);
  ...
  test("verify that throwing file not found exception when output location path is wrong") {
    assertThrows[FileNotFoundException] {
      var writtenStatus = superMovies.writeMovieDataBasedOnLogic(s"/Users/Documents/DMD/scala_file/Movie_Data_By_Rating.csv", result, HandleMoviesData.outputByHighestRating);
    }
  }
